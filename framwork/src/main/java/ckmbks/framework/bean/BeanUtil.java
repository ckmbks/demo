...
import ckmbks.framework.bean.*;
import ckmbks.framework.bean.BeanDesc;
import ckmbks.framework.bean.BeanDesc.PropDesc;
import ckmbks.framework.bean.BeanPath;
import ckmbks.framework.bean.copier.BeanCopier;
...
public class BeanUtil {
	...
	/**
	 * 获取{@link ckmbks.framework.bean.BeanDesc} Bean描述信息
	 * 
	 * @param clazz Bean类
	 * @return {@link ckmbks.framework.bean.BeanDesc}
	 * @since 3.1.2
	 */
	public static ckmbks.framework.bean.BeanDesc getBeanDesc(Class<?> clazz) {
		ckmbks.framework.bean.BeanDesc beanDesc = BeanDescCache.INSTANCE.getBeanDesc(clazz);
		if (null == beanDesc) {
	 ...
	 * @return Bean属性值
	 * @see ckmbks.framework.bean.BeanPath#get(Object)
	 * @since 3.0.7
	 */
	public static Object getProperty(Object bean, String expression) {
		return ckmbks.framework.bean.BeanPath.create(expression).get(bean);
	}
	 ...
	 * 如果isSimple为{@code false}，则只匹配类名而忽略包名，例如：ckmbks.framework.TestEntity只匹配TestEntity<br>
	 * 如果isSimple为{@code true}，则匹配包括包名的全类名，例如：ckmbks.framework.TestEntity匹配ckmbks.framework.TestEntity
	 * 
	 * @param bean Bean
