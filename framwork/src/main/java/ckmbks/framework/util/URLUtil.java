...
public class URLUtil {
	...
	public static URL toUrlForHttp(String urlStr, URLStreamHandler handler) {
		...
		urlStr = ckmbks.framework.util.StrUtil.cleanBlank(urlStr);
	...
	public static String complateUrl(String baseUrl, String relativePath) {
		...
		if (ckmbks.framework.util.StrUtil.isBlank(baseUrl)) {
			return null;
		}
	...
	public static String encode(String url, Charset charset) {
		if (ckmbks.framework.util.StrUtil.isEmpty(url)) {
			return url;
		}
		if (null == charset) {
	...
	public static String encode(String url, String charset) throws UtilException {
		...
		return encode(url, ckmbks.framework.util.StrUtil.isBlank(charset) ? CharsetUtil.defaultCharset() : CharsetUtil.charset(charset));
	}
	...
	public static String encodeQuery(String url, String charset) throws UtilException {
		return encodeQuery(url, ckmbks.framework.util.StrUtil.isBlank(charset) ? CharsetUtil.defaultCharset() : CharsetUtil.charset(charset));
	}
	...
	public static String normalize(String url, boolean isEncodeBody) {
		if(ckmbks.framework.util.StrUtil.isBlank(url)) {
			return url;
		}
		...
		String body;
		if(sepIndex > 0) {
			pre = ckmbks.framework.util.StrUtil.subPre(url, sepIndex + 3);
			body = ckmbks.framework.util.StrUtil.subSuf(url, sepIndex + 3);
		}else {
			pre = "http://";
			body = url;
		}
		
		final int paramsSepIndex = ckmbks.framework.util.StrUtil.indexOf(body, '?');
		String params = null;
		if(paramsSepIndex > 0) {
			params = ckmbks.framework.util.StrUtil.subSuf(body, paramsSepIndex);
			body = ckmbks.framework.util.StrUtil.subPre(body, paramsSepIndex);
		}
		
		//去除开头的\或者/
		body = body.replaceAll("^[\\/]+", ckmbks.framework.util.StrUtil.EMPTY);
		//替换多个\或/为单个/
