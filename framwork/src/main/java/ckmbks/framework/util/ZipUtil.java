...
import ckmbks.framework.io.FastByteArrayOutputStream;
...
public class ZipUtil {
	...
	public static byte[] gzip(String content, String charset) throws UtilException {
		return gzip(ckmbks.framework.util.StrUtil.bytes(content, charset));
	}
	...
	public static String unGzip(byte[] buf, String charset) throws UtilException {
		return ckmbks.framework.util.StrUtil.str(unGzip(buf), charset);
	}
	...
	public static byte[] zlib(String content, String charset, int level) {
		return zlib(ckmbks.framework.util.StrUtil.bytes(content, charset), level);
	}
	...
	public static String unZlib(byte[] buf, String charset) {
		return ckmbks.framework.util.StrUtil.str(unZlib(buf), charset);
	}
	...
	private static void zip(File file, String srcRootDir, ZipOutputStream out) throws UtilException {
		...
		if (file.isDirectory()) {// 如果是目录，则压缩压缩目录中的文件或子目录
			...
			if (ArrayUtil.isEmpty(files) && ckmbks.framework.util.StrUtil.isNotEmpty(subPath)) {
	...
	private static void addDir(String path, ZipOutputStream out) throws UtilException {
		path = ckmbks.framework.util.StrUtil.addSuffixIfNot(path, ckmbks.framework.util.StrUtil.SLASH);
		try {
			out.putNextEntry(new ZipEntry(path));
		} catch (IOException e) {
			throw new UtilException(e);
