...
import ckmbks.framework.convert.BasicType;
...
public class ClassUtil {
	...
	/**
	 * 获取完整类名的短格式如：<br>
	 * ckmbks.framework.util.StrUtil -》c.h.c.u.StrUtil
	 * 
	 * @param className 类名
	 * @return 短格式类名
	 * @since 4.1.9
	...
	public static String getShortClassName(String className) {
		final List<String> packages = ckmbks.framework.util.StrUtil.split(className, ckmbks.framework.util.CharUtil.DOT);
		if (null == packages || packages.size() < 2) {
			return className;
		}

		final int size = packages.size();
		final StringBuilder result = ckmbks.framework.util.StrUtil.builder();
		result.append(packages.get(0).charAt(0));
		for (int i = 1; i < size - 1; i++) {
			result.append(ckmbks.framework.util.CharUtil.DOT).append(packages.get(i).charAt(0));
		}
	...
	public static boolean equals(Class<?> clazz, String className, boolean ignoreCase) {
		if (null == clazz || ckmbks.framework.util.StrUtil.isBlank(className)) {
			return false;
		}
		if (ignoreCase) {
	...
	public static Field getDeclaredField(Class<?> clazz, String fieldName) throws SecurityException {
		if (null == clazz || ckmbks.framework.util.StrUtil.isBlank(fieldName)) {
			return null;
		}
		try {
	...
	public static Set<String> getClassPathResources(boolean isDecode) {
		return getClassPaths(ckmbks.framework.util.StrUtil.EMPTY, isDecode);
	}
	...
	public static Set<String> getClassPaths(String packageName, boolean isDecode) {
		String packagePath = packageName.replace(ckmbks.framework.util.StrUtil.DOT, ckmbks.framework.util.StrUtil.SLASH);
		...
		while (resources.hasMoreElements()) {
			...
			paths.add(isDecode ? ckmbks.framework.util.URLUtil.decode(path, CharsetUtil.systemCharsetName()) : path);
		}
		return paths;
	}
	...
	public static URL getClassPathURL() {
		return getResourceURL(ckmbks.framework.util.StrUtil.EMPTY);
	}
	...
	public static boolean isAllAssignableFrom(Class<?>[] types1, Class<?>[] types2) {
		if (ckmbks.framework.util.ArrayUtil.isEmpty(types1) && ArrayUtil.isEmpty(types2)) {
			return true;
		}
	...
	public static <T> T invoke(String classNameWithMethodName, boolean isSingleton, Object... args) {
		if (ckmbks.framework.util.StrUtil.isBlank(classNameWithMethodName)) {
	...
	public static String getPackage(Class<?> clazz) {
		if (clazz == null) {
			return ckmbks.framework.util.StrUtil.EMPTY;
		}
		final String className = clazz.getName();
		int packageEndIndex = className.lastIndexOf(ckmbks.framework.util.StrUtil.DOT);
	...
	public static String getPackagePath(Class<?> clazz) {
		return getPackage(clazz).replace(ckmbks.framework.util.StrUtil.C_DOT, StrUtil.C_SLASH);
	}
