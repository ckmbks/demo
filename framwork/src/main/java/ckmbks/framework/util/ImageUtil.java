...
public class ImageUtil {
	...
	public static void convert(File srcImageFile, File destImageFile) {
		...
		if (ckmbks.framework.util.StrUtil.equalsIgnoreCase(srcExtName, destExtName)) {
			// 扩展名相同直接复制文件
		...
		try {
			...
			convert(read(srcImageFile), destExtName, imageOutputStream, ckmbks.framework.util.StrUtil.equalsIgnoreCase(IMAGE_TYPE_PNG, srcExtName));
		} finally {
	...
	public static boolean write(Image image, String imageType, ImageOutputStream destImageStream, float quality) throws IORuntimeException {
		if (ckmbks.framework.util.StrUtil.isBlank(imageType)) {
	...
	public static Color hexToColor(String hex) {
		return getColor(Integer.parseInt(ckmbks.framework.util.StrUtil.removePrefix("#", hex), 16));
	}
	...
	public static Color getColor(String colorName) {
		if (ckmbks.framework.util.StrUtil.isBlank(colorName)) {
			return null;
		}
		...
		} else if (ckmbks.framework.util.StrUtil.startWith(colorName, '#')) {
