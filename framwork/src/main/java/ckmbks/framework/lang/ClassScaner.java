...
import ckmbks.framework.collection.EnumerationIter;
...
public class ClassScaner {
	...
	/** 过滤器 */
	private ckmbks.framework.lang.Filter<Class<?>> classFilter;
	/** 编码 */
	...
	public static Set<Class<?>> scanPackageByAnnotation(String packageName, final Class<? extends Annotation> annotationClass) {
		return scanPackage(packageName, new ckmbks.framework.lang.Filter<Class<?>>() {
			@Override
	 ...
	 * @return 类集合
	 */
	public static Set<Class<?>> scanPackage(String packageName, ckmbks.framework.lang.Filter<Class<?>> classFilter) {
	 ...
	 */
	public ClassScaner(String packageName, ckmbks.framework.lang.Filter<Class<?>> classFilter) {
	 ...
	 * @param charset 编码
	 */
	public ClassScaner(String packageName, ckmbks.framework.lang.Filter<Class<?>> classFilter, Charset charset) {
	...
	private void addIfAccept(String className) {
		...
		if(classLen == packageLen) {
			...
			//检查类名是否以指定包名为前缀，包名后加.（避免类似于ckmbks.framework.A和ckmbks.framework.ATest这类类名引起的歧义）
